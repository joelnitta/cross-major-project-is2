...
- まず、グーグルドライブから`屋上はらっぱ植生調査.csv`（生データ）をダウンロードして、`data/`に置いて下さい
...
```{r}
read_csv("data/屋上はらっぱ植生調査 - とりまとめ.csv")
```
...
- `read_csv()`関数の使い方を`?read_csv`で確認する
  - `?read_csv`はデータ解析に必要ではないコマンドなので、`roof-survey.R`に書かないで、直接コンソール（下の方のパネル）に入力
...
- `read_csv()`の主な引数（設定）
  - `file`：読み込むファイル名（場所）
  - `skip`：データを読み込み始めるまで最初に**読まない**（スキップ）する行の数
  - `n_max`：何行まで読み込む
...
- `quadrat`をもうちょっとよく見て下さい。どんなデータが入っている？

- `A-1`
  - `A`：プロット名
  - `1`: その中のクアドラット（quadrat）

- これをちゃんと分けましょう

---

- `separate_wider_delim()`を使う 
  - `separate`：分離させる
  - `wider`：より広く
  - `delim`：何かの記号を境目に
...
- これではまだ１区分しか読んでいない。他も同様に読み込む必要がある。
...
- 最後に、データを一つにまとめましょう
  - これで、やっと可視化（解析）できる段階に入った

```{r}
okujo <- bind_rows(okujo_a, okujo_b, okujo_c)
okujo <- read_csv("data/matome_clean.csv", col_types = "cccnnn")
```
...
ggplot(
  data = okujo,
  mapping = aes(x = plot,　y = cover,　color = plot)
) +
  geom_boxplot() +
  geom_jitter() +
  labs(
